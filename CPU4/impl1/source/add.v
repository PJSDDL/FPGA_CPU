...
/* Module Version: 3.5 */
/* D:\Diamond\diamond\3.12\ispfpga\bin\nt64\scuba.exe -w -n add -lang verilog -synth lse -bus_exp 7 -bb -arch xo2c00 -type add -width 12 -unsigned -port ci -port co -pipeline 0  */
/* Sat Jun 15 16:10:54 2024 */


`timescale 1 ns / 1 ps
module add (DataA, DataB, Cin, Result, Cout)/* synthesis NGD_DRC_MASK=1 */;
    ...
    FADD2B precin_inst41 (.A0(scuba_vlo), .A1(scuba_vlo), .B0(scuba_vlo), 
    ...
    FADD2B addsub_6 (.A0(DataA[11]), .A1(scuba_vlo), .B0(DataB[11]), .B1(scuba_vlo), 
        .CI(co5), .COUT(co6), .S0(Result[11]), .S1(Cout));

    VLO scuba_vlo_inst (.Z(scuba_vlo));

    FADD2B addsubd (.A0(scuba_vlo), .A1(scuba_vlo), .B0(scuba_vlo), .B1(scuba_vlo), 
        .CI(co6), .COUT(), .S0(co6d), .S1());
